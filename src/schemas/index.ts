...
export const CreateCampaignNoteRequestSchema = z.object({
  sessionId: z.string().uuid(),
  category: z.enum(['npcs', 'locations', 'plot', 'loot', 'clues']),
  title: z.string(),
  content: z.string(),
  metadata: z.record(z.string(), z.any()).optional(),
});
...
export const JournalSearchRequestSchema = z.object({
  ...
  entryType: z
    ...
    .optional(),
  category: z.enum(['npcs', 'locations', 'plot', 'loot', 'clues']).optional(),
  actorId: z.string().uuid().optional(),
  tags: z.array(z.string()).optional(),
...
// Multi-action penalty request schemas
export const SetMultiActionRequestSchema = z.object({
  sessionId: z.string().uuid(),
  actorId: z.string().uuid(),
  actions: z.number().min(1).max(3),
  description: z.string(),
});

export const CreateExtrasGroupRequestSchema = z.object({
  sessionId: z.string().uuid(),
  groupName: z.string(),
  actorIds: z.array(z.string().uuid()),
  sharedCard: z.boolean().default(true),
});
...
// Enhanced combat state with multi-action penalties and extras grouping
export const MultiActionPenaltySchema = z.object({
  actorId: z.string().uuid(),
  actions: z.number().min(1).max(3), // 1-3 actions per turn
  penalty: z.number().min(0).max(4), // -2 per additional action
  description: z.string(),
});
...
export const CombatStateResponseSchema = z.object({
  ...
  multiActionPenalties: z.array(MultiActionPenaltySchema).optional(),
  extrasGroups: z.array(ExtrasGroupSchema).optional(),
});
...
export type MultiActionPenalty = z.infer<typeof MultiActionPenaltySchema>;
